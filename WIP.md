...
적층 중이며, 현재 branch는 `fix/issue-555-state-store-job-lock-stacked`이다.
...
| [#555](https://github.com/bluetape4k/bluetape4k-graph/issues/555) | 구현·로컬 검증 완료, PR 준비 | #554 PR #578 exact head `26b41485d3107a99a555678de85fb455b1000504` 위에서 in-memory state store의 global monitor를 job별 reentrant lock registry로 좁혔다. implementation commit `030f7879`에 서로 다른 job 병렬성, 같은 job 직렬성, interrupt waiter 해제 회귀를 추가했으며 targeted 3/3·graph-io-core 154/154·Detekt·금지 assertion scan·diff-check를 통과했다. hosted 검증과 stacked PR 생성이 다음 단계다. |
