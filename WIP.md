...
`executeQuery()`/`ResultSet.next()` blocking 취소 회귀를 적층하는 다음 slice이며,
PR 생성 전 로컬 구현·검증을 진행한다. 최종 일괄 merge 승인 전에는 모든 PR을
독립 병합하지 않는다.
...
| [#552](https://github.com/bluetape4k/bluetape4k-graph/issues/552) | 구현·로컬 검증 진행, PR 준비 | #551 PR #574 exact head 위에서 AGE active JDBC statement를 cancellation 시작 시 한 번 cancel하고, Exposed cleanup의 statement/ResultSet close와 원래 `CancellationException` 전파를 deterministic proxy로 검증한다. 실제 driver가 `Statement.cancel()`을 지원하지 않는 경우의 timeout 한계는 EN/KO README와 7-Tier evidence에 기록한다. PR 생성 후 hosted exact-head 검증·리뷰를 대기한다. |
