...
| [#552](https://github.com/bluetape4k/bluetape4k-graph/issues/552) | PR #575 hosted 검증·리뷰 대기 | #551 PR #574 exact head 위에서 AGE active JDBC statement를 cancellation 시작 시 한 번 cancel하고, Exposed cleanup의 statement/ResultSet close와 원래 `CancellationException` 전파를 deterministic proxy로 검증했다. AGE 197 tests와 Detekt를 통과했으며 실제 driver가 `Statement.cancel()`을 지원하지 않는 경우의 timeout 한계를 EN/KO README와 7-Tier evidence에 기록했다. source implementation commit은 `35a9bef41daf5176a16695ee48cb15d7584e5344`, PR read-back docs commit은 `093f2a7cd09a3191965a869acc34ff5883d92379`이며, 최종 train merge는 마지막 승인 단계에서만 수행한다. |
