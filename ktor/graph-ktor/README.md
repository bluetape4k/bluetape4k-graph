...
- A failed plugin-owned close action remains retryable. Later `GraphPluginState.close()` calls retry only failed actions, while successful actions are not duplicated and concurrent calls serialize the complete close-action pass.
