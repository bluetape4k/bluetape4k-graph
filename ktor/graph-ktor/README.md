...
- A failed plugin-owned close action remains retryable. Later `GraphPluginState.close()` calls retry only failed actions, while successful actions are not duplicated. Concurrent calls coalesce into the in-flight close-action pass and return without waiting for it.
