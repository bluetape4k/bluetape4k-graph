...
- Plugin-owned close action이 실패하면 재시도 가능한 상태를 유지합니다. 이후 `GraphPluginState.close()`는 실패한 action만 재시도하고 성공한 action은 중복 실행하지 않으며, 동시 호출은 전체 close-action pass를 직렬화합니다.
