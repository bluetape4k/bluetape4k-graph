...
[시작하기](../getting-started.md)를 TinkerGraph로 실행한다. 불투명한 ID, 방향이 있는 간선, 반환값이 스냅샷이라는 점을 익힌다. 생성된 ID와 이웃 방향을 관찰한다. 이웃이 안 나오면 `startId`/`endId`, `Direction`, label 조건을 확인한다. 이어서 [`GraphVertexTest.kt`](https://github.com/bluetape4k/bluetape4k-graph/blob/3e0fa7cb9e3bc70c2743aeebda2487f3e45e4907/graph/graph-core/src/test/kotlin/io/bluetape4k/graph/model/GraphVertexTest.kt)를 읽는다.
