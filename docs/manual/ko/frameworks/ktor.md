...
managed `neo4j { ... }`는 operations와 Driver를 만들고 `ApplicationStopped`에서 닫는다. 호출자가 Driver를 만들었다면 동기·코루틴 operations를 구성해 `operations(sync, suspend)`로 넘긴다. plugin은 operations close를 중복 실행하지 않지만 Driver는 호출자가 닫는다.
