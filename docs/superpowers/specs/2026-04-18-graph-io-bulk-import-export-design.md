...
- **Modules**: `graph-io-core`, `graph-io-csv`, `graph-io-jackson2`, `graph-io-jackson3`, `graph-io-graphml`, `graph-io-benchmark`
...
graph-io/csv/       ← CSV 포맷 (:graph-io-csv)
graph-io/jackson2/  ← Jackson2 NDJSON (:graph-io-jackson2)
graph-io/jackson3/  ← Jackson3 NDJSON (:graph-io-jackson3)
graph-io/graphml/   ← GraphML (:graph-io-graphml)
benchmark/graph-io-benchmark/ ← IO 전용 벤치마크 (:graph-io-benchmark)
```

Pros:

- `graph-io-core`는 순수 공통 계약과 report/model만 가진다.
...
```
graph-io/
  core/                        ← 공통 계약 모듈 (:graph-io-core)
  csv/                         ← CSV 포맷 모듈 (:graph-io-csv)
  jackson2/                    ← Jackson 2 NDJSON 포맷 모듈 (:graph-io-jackson2)
  jackson3/                    ← Jackson 3 NDJSON 포맷 모듈 (:graph-io-jackson3)
  graphml/                     ← GraphML 포맷 모듈 (:graph-io-graphml)
benchmark/
  graph-io-benchmark/          ← IO 전용 벤치마크 모듈 (:graph-io-benchmark)
```
...
| `benchmark/graph-io-benchmark/` | `:graph-io-benchmark` |

`settings.gradle.kts`에서 `includeModules("graph-io", false, true)` 와 `includeModules("benchmark", false, false)` 로 자동 등록된다.

### 5.1 `graph-io-core` (`graph-io/core/`)
