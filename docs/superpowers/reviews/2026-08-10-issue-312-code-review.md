...
| 종료/관찰성 | PASS | cancellation hook exactly-once, 독립 close-grace bounded cancellation/observer/rollback, active load/take interrupt, terminal diagnostic barrier, interrupt flag 복원, diagnostic single-inflight 및 pending timeout retry를 확인했다. `Error`/raw `Throwable`도 fixed code로 redaction된다. |
...
- GREEN targeted: `GraphNativeBulkLoadModelsTest`와
  `GraphNativeBulkLoaderTest` **21개 통과**.
- 전체 core: `:bluetape4k-graph-io-core:test` **128개 통과**.
...
- detekt: root `./gradlew detekt` **통과**. nativebulk 파일은 의도적 API 복잡도
  suppression과 한국어 public KDoc으로 정리했다.
