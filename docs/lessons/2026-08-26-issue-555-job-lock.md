...
- candidate head `5260dc89f16af9503c5c9d1b4016c3c14ceab9ea`에서 hosted CI
  `32899855369`와 Examples `32899855380`이 모두 성공했다. docs-only lifecycle
  sync 뒤 새 final head의 checks와 PR metadata를 다시 읽는다.
...
3. docs-only lifecycle sync 후 exact final head hosted CI·Examples와
   metadata/stacked base를 read-back하고, 전체 train 마지막 승인 전에는
   merge하지 않는다.
