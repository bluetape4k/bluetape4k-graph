...
class CachingMemgraphGraphOperationsTest {
    ...
    fun `maxSize는 단일 read cache의 엔트리 수를 제한한다`() {
        val bounded = CachingMemgraphGraphOperations(delegate, maxSize = 1)
        every { delegate.findVertexById("Person", aliceId) } returns alice
    ...
    fun `expireAfterWrite 후 read cache가 만료되어 delegate를 다시 호출한다`() {
        val expiring = CachingMemgraphGraphOperations(delegate, expireAfterWrite = Duration.ofMillis(20))
        every { delegate.findVertexById("Person", aliceId) } returns alice
    ...
    fun `maxSize는 양수여야 한다`() {
        assertFailsWith<IllegalArgumentException> {
            CachingMemgraphGraphOperations(delegate, maxSize = 0)
        }
    }
    ...
    fun `expireAfterWrite는 양수여야 한다`() {
        assertFailsWith<IllegalArgumentException> {
            CachingMemgraphGraphOperations(delegate, expireAfterWrite = Duration.ZERO)
        }
    }
}
