...
import io.bluetape4k.graph.spring.boot3.properties.FalkorDBGraphProperties
...
class GraphFalkorDBAutoConfiguration {
    ...
    class HealthConfig {
        ...
        fun falkordbHealthIndicator(driver: com.falkordb.Driver): org.springframework.boot.actuate.health.HealthIndicator =
            org.springframework.boot.actuate.health.HealthIndicator {
                try {
                    driver.graph("__health__").use { it.query("RETURN 1") }
                    org.springframework.boot.actuate.health.Health.up().withDetail("backend", "falkordb").build()
                } catch (e: Exception) {
