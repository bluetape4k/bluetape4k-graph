...
`NdJsonReadOptions.maxLineChars`는 Jackson codec이 보기 전에 NDJSON 한 줄의 길이를
제한합니다. 단위는 UTF-16 code unit이고 값은 양수여야 하며, 기본값
`Int.MAX_VALUE`는 기존의 사실상 제한 없는 입력 동작을 유지합니다. Jackson2와
Jackson3의 sync, suspend, Virtual Thread, Flow reader가 같은 immutable 옵션을
사용하고, 설정한 값은 checkpoint 호환성에도 포함됩니다.
...
- **`GraphImportWorkflow` / `GraphImportJobStateStore`** — multi-source 임포트 manifest를 검증하고 순서가 있는 job state를 저장합니다. store의 `update` 경계는 한 JVM store 인스턴스에서 load/검증/save를 원자적으로 수행하며, 전이할 때 `copy(state = ...)`를 사용해 기존 `sources`·`elapsed`·`checkpoint` payload를 보존합니다. transform은 순수하고 retry-safe해야 하며 durable store는 native transaction 또는 CAS로 override해야 합니다. in-memory reference store는 job별 interruptible·참조 카운트 reentrant lock을 사용하므로 서로 다른 job은 병렬로 진행하고 같은 job 전이는 직렬로 유지하며 idle lock entry를 제거합니다. 이는 JVM-local 동작이며 durable multi-process 보장은 아닙니다. 재사용 가능한 state-store contract TCK는 Gradle `testFixtures` variant로 제공합니다.
