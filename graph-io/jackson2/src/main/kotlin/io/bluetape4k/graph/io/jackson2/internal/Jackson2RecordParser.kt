...
internal data class Jackson2ParsedRecord(
    val lineNumber: Int,
    val envelope: NdJsonEnvelope,
)

/** Jackson2 NDJSON를 한 줄씩 해석하고 source ownership을 관리하는 내부 parser. */
internal class Jackson2RecordParser(
    ...
    ): Flow<Jackson2ParsedRecord> = channelFlow {
        val producer = this
                ...
                onRecord = { record ->
                    ...
                    if (result.isFailure) {
                        if (!producer.coroutineContext.isActive) {
                            throw CancellationException("Jackson2 NDJSON collection cancelled")
                        }
                        throw result.exceptionOrNull()
                            ?: IllegalStateException("Jackson2 NDJSON channel closed")
                    }
                },
    ...
    fun parse(
        source: GraphImportSource,
        onRecord: (Jackson2ParsedRecord) -> Unit,
        ...
        try {
            GraphIoPaths.openReader(source).use { reader ->
                while (parsing) {
                    ...
                    try {
                        onRecord(Jackson2ParsedRecord(lineNumber, envelope))
